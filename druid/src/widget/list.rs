...
pub struct List<T: Data, S> {
    closure: Box<dyn Fn() -> Box<dyn Widget<T, S>>>,
    children: Vec<WidgetPod<T, S, Box<dyn Widget<T, S>>>>,
}

impl<T: Data, S> List<T, S> {
    /// Create a new list widget. Closure will be called every time when a new child
    /// needs to be constructed.
    pub fn new<W: Widget<T, S> + 'static>(closure: impl Fn() -> W + 'static) -> Self {
        List {
